...
const TypeTab: FC<{}> = () => {
  ...
  const handleChange = (event: ChangeEvent<HTMLInputElement>) => {
    if (types && types.allTypes) {
      const selectedType = types.allTypes.find((item) => item.id === event.target.value);
  ...
  return types ? (
    <Stack gap="10px" padding="10px">
      ...
      <Typography color="textSecondary">
        In the following list is shown the frequency
        of the types which are present in the column
      </Typography>
      {selected && (
        <Box
          marginLeft="-10px"
          ...
          <Stack component={SquaredBox}>
            <Typography variant="h6" fontSize="14px">Current selection</Typography>
            <RadioLabel checked {...selected} />
          </Stack>
        </Box>
      )}
      <RadioButtonsGroup types={types.allTypes} value={selected ? selected.id : ''} onChange={handleChange} />
    </Stack>
  ) : null;
};
