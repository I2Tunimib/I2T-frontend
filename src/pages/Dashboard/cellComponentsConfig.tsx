...
export const CELL_COMPONENTS_TYPES: Record<CellComponent, CellComponentProps> =
  {
    link: {
      component: (cell, { props }) => {
        ...
        return (
          <MatLink sx={{ textDecoration: "none" }} component={Link} to={url}>
            {value}
          </MatLink>
        );
      },
    ...
    percentage: {
      ...
      sortFn: (rowA: any, rowB: any, columnId: string, desc: boolean) => {
        const { total: totalA, value: valueA } = rowA.getValue(columnId);
        const { total: totalB, value: valueB } = rowB.getValue(columnId);
        if (totalA === 0 || totalB === 0) return 0;
        return valueA / totalA < valueB / totalB ? -1 : 1;
      },
    },
  };
